    ...
    return (
        <div className="max-w-xl mx-auto pb-20">
            <div className="sticky top-0 z-10 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-6 mb-6 flex items-center gap-4">
                ...
                <h1 className="text-xl font-black text-gray-900">{t('following')}</h1>
            </div>
            ...
            <div className="px-4 space-y-2">
                {loading ? (
                    ...
                    <p className="text-center py-20 text-gray-400 font-bold">まだ誰もフォローしていません 🦴</p>
                ) : (
                    following.map((f) => (
                        <Link
                            key={f.followed.id}
                            href={`/app/dog/${f.followed.id}`}
                        ...
                        >
                            <div className="w-12 h-12 rounded-xl overflow-hidden bg-green-50 flex items-center justify-center border-2 border-green-50 shadow-sm group-hover:scale-105 transition-transform">
                                {f.followed.iconUrl ? (
                                    <img src={f.followed.iconUrl} alt={f.followed.name} className="w-full h-full object-cover" />
                                ) : (
                                    <span className="text-2xl">🐶</span>
                                )}
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="font-black text-gray-900 truncate">{f.followed.name}</h3>
                                <p className="text-[10px] font-bold text-green-600 uppercase tracking-widest">{f.followed.breed}</p>
                            </div>
